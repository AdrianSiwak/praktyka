...
def multiple_calculations(min_x,max_x,step,R): 
     """funkcja wykonuje operację
        funkcji calculate_ONP_with_x wielokrotnie, 
        na zadanymprzedziale z krokiem, zwraca listę wyników """
     calculations=[]
     for i in np.linspace(min_x,max_x,step):
        calculations.append(calculate_ONP_with_x(R,i))
     return calculations
...
#input_string="x-1"#input()
#tokens=string_to_tokens(input_string)
#Q=ONP(tokens)
#print(Q)
#print(''.join([str(elem) for elem in Q]))
#print(multiple_calculations(float(1), float(2), int(3), Q))
